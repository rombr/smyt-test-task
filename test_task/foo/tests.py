...
class LoadConfigTestCase(TestCase):
    ...
    def test_invalid_config_file_format(self):
        self.assertRaises(InvalidConfigFileFormatError, get_models_config, 'test_configs/models.json')

    def test_invalid_config_structure(self):
        self.assertRaises(ConfigFileParseError, get_models_config, 'test_configs/models_invalid.yaml')


class ModelsTestCase(TestCase):
    def setUp(self):
        Users.objects.create(name="Tom Backs", paycheck=100500, date_joined="2013-12-21")
    ...
    def test_invalid_date(self):
        self.assertRaises(ValidationError, Users.objects.create, name="Tom Backs", paycheck=100500, date_joined="2013-12-21-")
...
class OperationsTestCase(TestCase):
    ...
    def test_api_create_new_object(self):
        response = self.client.post(reverse('api', args=['rooms']), {'department':"Sales", 'spots':15, })
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Sales")

    def test_api_create_new_object_with_bad_param(self):
        response = self.client.post(reverse('api', args=['rooms']), {'department':"Sales", 'spots':'15d', })
    ...
    def test_api_update_object_details(self):
        response = self.client.post(reverse('api_details', args=['users', 1]), {'name': 'Mark Grace', })
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Mark Grace")

    def test_api_update_object_details_with_invalid_param(self):
        response = self.client.post(reverse('api_details', args=['users', 1]), {'date_joined': '2013-12-21@', })
        self.assertEqual(response.status_code, 400)
